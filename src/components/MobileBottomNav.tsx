...
export function MobileBottomNav() {
  ...
  const items = [
    { to: "/workspace", icon: LayoutDashboard, label: t("dashboard"), end: true, show: isModuleEnabled("dashboard") },
  ...
  return (
    ...
    >
      {items.map(it => (
        <NavLink
          key={it.to}
          to={currentBranchId ? `${it.to}?branch=${encodeURIComponent(currentBranchId)}` : it.to}
          end={it.end}
