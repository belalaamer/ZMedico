  ...
  return (
    <div dir={isAr ? "rtl" : "ltr"} className="min-h-dvh bg-background text-foreground">
      <header className="sticky top-0 z-20 border-b bg-background/95 backdrop-blur">
        ...
        {menuOpen ? <div className="border-t px-4 py-4 sm:hidden"><div className="flex flex-col gap-3 text-sm"><a href="#features" onClick={() => setMenuOpen(false)}>{isAr ? "المزايا" : "Features"}</a><a href="#how-it-works" onClick={() => setMenuOpen(false)}>{isAr ? "كيف تعمل" : "How it works"}</a><Link to="/pricing" onClick={() => setMenuOpen(false)}>{isAr ? "الأسعار" : "Pricing"}</Link><div className="flex flex-col gap-2 pt-2"><Button variant="outline" size="sm" className="w-full" onClick={() => setLang(isAr ? "en" : "ar")}><Globe2 className="me-2 size-4" />{isAr ? "English" : "العربية"}</Button><Button asChild variant="outline" size="sm" className="w-full"><Link to="/auth" onClick={() => setMenuOpen(false)}>{isAr ? "تسجيل الدخول" : "Sign in"}</Link></Button><Button asChild size="sm" className="w-full"><Link to="/request-trial" onClick={() => setMenuOpen(false)}>{isAr ? "ابدأ تجربتك" : "Start your trial"}</Link></Button></div></div></div> : null}
      </header>
