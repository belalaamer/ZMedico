...
import { CheckCircle2, DownloadCloud, KeyRound, Link2, Loader2, Plus, RefreshCw, ShieldCheck, Unplug, XCircle } from "lucide-react";
...
type SyncResponse = { ok?: boolean; spend_rows?: number; total_spend?: number; leads_seen?: number; leads_inserted?: number; leads_error?: string | null; error?: string };
...
async function functionErrorMessage(error: unknown, data: { error?: string } | undefined, fallback: string) {
...
export default function MetaAdsConnection() {
  ...
  const syncNow = async (connection: Connection) => {
    if (syncingId) return;
    setSyncingId(connection.id);
    const { data, error } = await supabase.functions.invoke<SyncResponse>("meta-ads-sync", { body: { action: "sync", branch_id: connection.branch_id } });
    if (error || data?.error) {
      toast.error(await functionErrorMessage(error, data, isArabic ? "فشلت مزامنة Meta" : "Meta sync failed"));
    } else {
      const spendPart = isArabic ? `${data?.spend_rows ?? 0} صف إنفاق` : `${data?.spend_rows ?? 0} spend rows`;
      const leadsPart = isArabic ? `${data?.leads_inserted ?? 0} عميل جديد` : `${data?.leads_inserted ?? 0} new leads`;
      toast.success(isArabic ? `تمت المزامنة: ${spendPart} · ${leadsPart}` : `Synced: ${spendPart} · ${leadsPart}`);
      if (data?.leads_error) {
        toast.warning(isArabic
          ? `الإنفاق تزامن، لكن جلب عملاء النماذج فشل: ${data.leads_error} — غالبًا الـToken يحتاج صلاحية leads_retrieval.`
          : `Spend synced, but pulling form leads failed: ${data.leads_error} — the token likely needs the leads_retrieval permission.`);
      }
    }
    await load();
    setSyncingId(null);
