-- 1) Slice gate row for patients (upsert-safe).
INSERT INTO public.authz_shadow_slice_gate (
  ...
  'patients',
  ARRAY['patients.view','patients.create','patients.edit','patients.delete','patients.export']::text[],
  ARRAY['admin','manager','receptionist']::app_role[],
  ARRAY['staff']::app_role[],
  ARRAY['bundle.role.admin','bundle.role.manager','bundle.role.receptionist']::text[],
  ARRAY['bundle.role.staff']::text[],
  'Patients vertical slice — Migration 1 shadow. Legacy patients module actions map 1:1 to canonical keys; no intentional expansions.'
)
...
-- 2) Patients matrix view — same expression as v_authz_shadow_matrix_settings,
--    filtered to slice='patients'.
CREATE OR REPLACE VIEW public.v_authz_shadow_matrix_patients AS
WITH gate AS (
  SELECT g.required_keys,
         g.required_write_roles || g.required_denied_roles AS all_roles
  FROM public.authz_shadow_slice_gate g
  WHERE g.slice = 'patients'
),
cells AS (
  ...
  SELECT c.role,
         ...
         (
           COALESCE((
             ...
             WHERE rp.role = c.role AND rp.module = 'patients'
             LIMIT 1
           ), false)
           OR EXISTS (
             SELECT 1
             FROM public.authz_shadow_expected_expansions e
             WHERE e.slice = 'patients'
...
GRANT SELECT ON public.v_authz_shadow_matrix_patients TO authenticated;
GRANT ALL    ON public.v_authz_shadow_matrix_patients TO service_role;

-- 3) Extend exit criteria — add matrix_patients CTE and patients branch.
--    Settings branch preserved byte-for-byte.
...
SELECT g.slice,
       ...
       (
         ...
         AND (
           g.slice NOT IN ('settings','patients')
           OR (
