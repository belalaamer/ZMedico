-- 1) Slice gate row for medical_records (upsert-safe).
INSERT INTO public.authz_shadow_slice_gate (
  slice,
  required_keys,
  required_write_roles,
  required_denied_roles,
  required_granting_bundles,
  required_denying_bundles,
  notes
) VALUES (
  'medical_records',
  ARRAY['medical_records.view','medical_records.create','medical_records.edit','medical_records.delete','medical_records.export']::text[],
  ARRAY['admin','doctor']::app_role[],
  ARRAY['staff']::app_role[],
  ARRAY['bundle.role.admin','bundle.role.doctor']::text[],
  ARRAY['bundle.role.staff']::text[],
  'Medical Records vertical slice — Migration 1 shadow. Legacy medical_records module actions map 1:1 to canonical keys; no intentional expansions.'
)
...
-- 2) Medical Records matrix view — same expression as v_authz_shadow_matrix_patients,
--    filtered to slice='medical_records'.
CREATE OR REPLACE VIEW public.v_authz_shadow_matrix_medical_records AS
WITH gate AS (
  SELECT g.required_keys,
         g.required_write_roles || g.required_denied_roles AS all_roles
  FROM public.authz_shadow_slice_gate g
  WHERE g.slice = 'medical_records'
),
cells AS (
  ...
  SELECT c.role,
         ...
         (
           COALESCE((
             ...
             WHERE rp.role = c.role AND rp.module = 'medical_records'
             LIMIT 1
           ), false)
           OR EXISTS (
             SELECT 1
             FROM public.authz_shadow_expected_expansions e
             WHERE e.slice = 'medical_records'
...
GRANT SELECT ON public.v_authz_shadow_matrix_medical_records TO authenticated;
GRANT ALL    ON public.v_authz_shadow_matrix_medical_records TO service_role;

-- 3) Extend exit criteria — add matrix_medical_records CTE and branch.
--    Settings + Patients branches preserved byte-for-byte.
...
observed_keys AS (
  SELECT d.slice,
         array_agg(DISTINCT d.permission_key) AS keys
  FROM public.authz_shadow_decisions d
  GROUP BY d.slice
),
observed_granting_bundles AS (
  SELECT d.slice,
         array_agg(DISTINCT rb.bundle_key) AS bundles
  FROM public.authz_shadow_decisions d
  JOIN public.user_roles ur ON ur.user_id = d.user_id
...
parity AS (
  SELECT p.slice,
         p.total_decisions,
         p.regressions,
         p.unexpected_expansions,
         p.expected_expansions,
         p.matches,
         p.unique_keys,
         p.unique_users,
         p.legacy_allow,
...
SELECT g.slice,
       ...
       (
         ...
         AND (
           g.slice NOT IN ('settings','patients','medical_records')
           OR (
             g.slice = 'settings' AND (
               COALESCE((SELECT pos_missing FROM matrix_settings), 0::bigint) = 0
               AND COALESCE((SELECT pos_red     FROM matrix_settings), 0::bigint) = 0
               AND COALESCE((SELECT neg_missing FROM matrix_settings), 0::bigint) = 0
               AND COALESCE((SELECT neg_red     FROM matrix_settings), 0::bigint) = 0
             )
           )
           OR (
             g.slice = 'patients' AND (
               COALESCE((SELECT pos_missing FROM matrix_patients), 0::bigint) = 0
               AND COALESCE((SELECT pos_red     FROM matrix_patients), 0::bigint) = 0
               AND COALESCE((SELECT neg_missing FROM matrix_patients), 0::bigint) = 0
               AND COALESCE((SELECT neg_red     FROM matrix_patients), 0::bigint) = 0
             )
           )
           OR (
             g.slice = 'medical_records' AND (
               COALESCE((SELECT pos_missing FROM matrix_medical_records), 0::bigint) = 0
               AND COALESCE((SELECT pos_red     FROM matrix_medical_records), 0::bigint) = 0
               AND COALESCE((SELECT neg_missing FROM matrix_medical_records), 0::bigint) = 0
               AND COALESCE((SELECT neg_red     FROM matrix_medical_records), 0::bigint) = 0
             )
           )
         )
