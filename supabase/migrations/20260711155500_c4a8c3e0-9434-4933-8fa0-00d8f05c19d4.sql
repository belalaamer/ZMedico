-- HR vertical slice — Migration 1 (shadow).
-- 1) Slice gate row.
INSERT INTO public.authz_shadow_slice_gate (
  ...
  'hr',
  ARRAY['hr.view','hr.create','hr.edit','hr.delete','hr.export']::text[],
  ARRAY['admin','hr']::app_role[],
  ARRAY['staff']::app_role[],
  ARRAY['bundle.role.admin','bundle.role.hr']::text[],
  ARRAY['bundle.role.staff']::text[],
  'HR vertical slice — Migration 1 shadow. Legacy hr module actions map 1:1 to canonical keys; no intentional expansions.'
)
...
-- 2) HR matrix view — mirrors v_authz_shadow_matrix_medical_records, filtered to slice='hr'.
CREATE OR REPLACE VIEW public.v_authz_shadow_matrix_hr AS
WITH gate AS (
  ...
  SELECT c.role,
         ...
         (
           COALESCE((
             ...
             WHERE rp.role = c.role AND rp.module = 'hr'
             LIMIT 1
           ), false)
           OR EXISTS (
             SELECT 1
...
GRANT SELECT ON public.v_authz_shadow_matrix_hr TO authenticated;
GRANT ALL    ON public.v_authz_shadow_matrix_hr TO service_role;

-- 3) Extend v_authz_shadow_exit_criteria — add matrix_hr CTE + WHEN branch.
--    Settings + Patients + Medical Records branches preserved byte-for-byte.
