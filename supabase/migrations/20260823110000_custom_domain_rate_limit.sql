-- Durable actor/action rate limiter for the low-volume platform custom-domain API.
CREATE TABLE IF NOT EXISTS public.custom_domain_rate_limits (
  actor_id uuid NOT NULL,
  action text NOT NULL,
  window_started_at timestamptz NOT NULL,
  request_count integer NOT NULL DEFAULT 0,
  PRIMARY KEY (actor_id, action, window_started_at),
  CONSTRAINT custom_domain_rate_limits_action_check CHECK (action IN ('list', 'create', 'status', 'disable', 'remove')),
  CONSTRAINT custom_domain_rate_limits_count_check CHECK (request_count >= 0)
);

ALTER TABLE public.custom_domain_rate_limits ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.custom_domain_rate_limits FROM PUBLIC, anon, authenticated;
...
DECLARE
  ...
  v_count integer;
BEGIN
  IF _actor_id IS NULL OR _action NOT IN ('list', 'create', 'status', 'disable', 'remove') THEN
    RETURN false;
  END IF;
