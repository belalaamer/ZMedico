-- Patient Flow / Self Check-in v1.
-- Raw tokens are returned only at creation time; only SHA-256 hashes are stored.
CREATE EXTENSION IF NOT EXISTS pgcrypto;

ALTER TABLE public.appointments
  ADD COLUMN IF NOT EXISTS self_checked_in_at timestamptz NULL,
  ADD COLUMN IF NOT EXISTS check_in_method text NULL
    CHECK (check_in_method IS NULL OR check_in_method IN ('staff', 'self'));

CREATE TABLE IF NOT EXISTS public.self_checkin_tokens (
...
CREATE INDEX IF NOT EXISTS self_checkin_tokens_active_idx
  ON public.self_checkin_tokens (appointment_id, expires_at)
  WHERE revoked_at IS NULL;

ALTER TABLE public.self_checkin_tokens ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.self_checkin_tokens FROM anon, authenticated;
...
BEGIN
  ...
  VALUES (a.id, encode(digest(raw_token, 'sha256'), 'hex'), token_expiry, auth.uid());
...
BEGIN
  ...
  SELECT a.id, a.branch_id, a.status, a.checked_in_at, a.deleted_at, b.tenant_id,
         t.is_active AS tenant_is_active, t.subscription_status, t.subscription_ends_at, t.trial_ends_at
    INTO a
    FROM public.appointments a
    JOIN public.branches b ON b.id = a.branch_id
    JOIN public.tenants t ON t.id = b.tenant_id
