-- pgcrypto compatibility fix: cast token bytes and algorithm explicitly.
...
BEGIN
  ...
  VALUES (a.id, encode(digest(convert_to(raw_token, 'UTF8'), 'sha256'::text), 'hex'), token_expiry, auth.uid());
...
BEGIN
  ...
  SELECT true,
   ...
   WHERE s.token_hash = encode(digest(convert_to(lower(p_token), 'UTF8'), 'sha256'::text), 'hex')
