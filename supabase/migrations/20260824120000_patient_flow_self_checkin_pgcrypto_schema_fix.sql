-- pgcrypto schema qualification fix for Supabase extensions schema.
...
BEGIN
  ...
  VALUES (a.id, encode(extensions.digest(convert_to(raw_token, 'UTF8'), 'sha256'::text), 'hex'), token_expiry, auth.uid());
...
BEGIN
  ...
  SELECT true,
   ...
   WHERE s.token_hash = encode(extensions.digest(convert_to(lower(p_token), 'UTF8'), 'sha256'::text), 'hex')
