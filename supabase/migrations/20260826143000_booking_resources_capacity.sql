-- Booking resources and capacity for multi-room clinics.
-- Backward compatible: branches without configured resources keep using
-- appointment_settings.max_appointments_per_slot.
...
CREATE TABLE IF NOT EXISTS public.booking_resources (
  ...
  resource_type text NOT NULL DEFAULT 'room' CHECK (resource_type IN ('room','equipment','provider')),
...
CREATE TABLE IF NOT EXISTS public.booking_resource_services (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (resource_id, service_id, service_kind)
);
...
ALTER TABLE public.appointments
  ...
  FOREIGN KEY (resource_id) REFERENCES public.booking_resources(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_booking_resources_branch_active
  ON public.booking_resources(branch_id, is_active, display_order);
CREATE INDEX IF NOT EXISTS idx_booking_resource_services_service
  ON public.booking_resource_services(service_id, service_kind);
CREATE INDEX IF NOT EXISTS idx_appointments_resource_time
  ON public.appointments(resource_id, scheduled_at)
...
DROP POLICY IF EXISTS booking_resources_branch_access ON public.booking_resources;
CREATE POLICY booking_resources_branch_access ON public.booking_resources
...
DROP POLICY IF EXISTS booking_resources_manage ON public.booking_resources;
CREATE POLICY booking_resources_manage ON public.booking_resources
  FOR ALL TO authenticated
  USING (public.user_has_branch_access(branch_id) AND (public.has_role(auth.uid(),'admin'::public.app_role) OR public.has_role(auth.uid(),'manager'::public.app_role)))
  WITH CHECK (public.user_has_branch_access(branch_id) AND (public.has_role(auth.uid(),'admin'::public.app_role) OR public.has_role(auth.uid(),'manager'::public.app_role)));

DROP POLICY IF EXISTS booking_resource_services_branch_access ON public.booking_resource_services;
CREATE POLICY booking_resource_services_branch_access ON public.booking_resource_services
  AS RESTRICTIVE FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.booking_resources r WHERE r.id = resource_id AND public.user_has_branch_access(r.branch_id)))
  WITH CHECK (EXISTS (SELECT 1 FROM public.booking_resources r WHERE r.id = resource_id AND public.user_has_branch_access(r.branch_id)));

DROP POLICY IF EXISTS booking_resource_services_manage ON public.booking_resource_services;
CREATE POLICY booking_resource_services_manage ON public.booking_resource_services
  FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM public.booking_resources r WHERE r.id = resource_id AND public.user_has_branch_access(r.branch_id) AND (public.has_role(auth.uid(),'admin'::public.app_role) OR public.has_role(auth.uid(),'manager'::public.app_role))))
  WITH CHECK (EXISTS (SELECT 1 FROM public.booking_resources r WHERE r.id = resource_id AND public.user_has_branch_access(r.branch_id) AND (public.has_role(auth.uid(),'admin'::public.app_role) OR public.has_role(auth.uid(),'manager'::public.app_role))));

CREATE OR REPLACE FUNCTION public.booking_resource_is_eligible(
  p_resource_id uuid,
  p_service_id uuid,
  p_service_kind text
)
...
AS $$
  SELECT NOT EXISTS (
    SELECT 1 FROM public.booking_resource_services brs
    WHERE brs.service_id = p_service_id
      AND brs.service_kind = p_service_kind
  )
  OR EXISTS (
    SELECT 1 FROM public.booking_resource_services brs
    WHERE brs.resource_id = p_resource_id
      AND brs.service_id = p_service_id
      AND brs.service_kind = p_service_kind
  );
$$;
