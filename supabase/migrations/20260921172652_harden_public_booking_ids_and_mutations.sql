-- Harden public booking boundaries:
-- 1) Do not expose internal appointment/patient/doctor ids in anonymous booking responses.
-- 2) Disable legacy anonymous mutation/lookup RPCs that rely only on internal identifiers.
...
REVOKE EXECUTE ON FUNCTION public.public_cancel_appointment_for_tenant(uuid, uuid, text) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.public_reschedule_appointment_for_tenant(uuid, uuid, timestamptz) FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.public_find_patient_by_phone_for_tenant(uuid, uuid, text) FROM anon, authenticated;
